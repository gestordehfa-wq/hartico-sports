...
alter table racing.circuits enable row level security;
create trigger circuits_updated_at before update on racing.circuits for each row execute function racing.set_updated_at();

create table racing.grand_prix_events (
  id uuid primary key default extensions.gen_random_uuid(),
  season_id uuid not null references racing.seasons(id) on delete restrict,
  circuit_id uuid not null references racing.circuits(id) on delete restrict,
...
alter table racing.grand_prix_events enable row level security;
create index grand_prix_events_calendar_idx on racing.grand_prix_events (season_id, scheduled_date, round_number);
create trigger grand_prix_events_updated_at before update on racing.grand_prix_events for each row execute function racing.set_updated_at();

create view racing.grand_prix_calendar
...
comment on column racing.grand_prix_events.race_laps is 'Override opcional; la carrera futura usa este valor o circuits.default_laps, siempre entre 4 y 8.';
