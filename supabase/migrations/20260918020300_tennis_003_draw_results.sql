...
begin
  ...
  select draw_size into size from tennis.tournament_editions where id = new.tournament_edition_id;
  if new.seed > size then raise exception 'seed outside draw size'; end if;
  return new;
end;
$$;
revoke all on function tennis.validate_entry_seed() from public, anon, authenticated;
create trigger tournament_entries_seed before insert or update on tennis.tournament_entries
for each row execute function tennis.validate_entry_seed();

create or replace function tennis.validate_match_integrity()
returns trigger language plpgsql set search_path = '' as $$
declare target tennis.matches%rowtype;
begin
  if new.player1_id is not null and not exists (select 1 from tennis.tournament_entries where tournament_edition_id = new.tournament_edition_id and player_id = new.player1_id and entry_status in ('registered', 'active')) then raise exception 'player1 is not an active edition entry'; end if;
  if new.player2_id is not null and not exists (select 1 from tennis.tournament_entries where tournament_edition_id = new.tournament_edition_id and player_id = new.player2_id and entry_status in ('registered', 'active')) then raise exception 'player2 is not an active edition entry'; end if;
  if new.next_match_id is not null then
    select * into target from tennis.matches where id = new.next_match_id;
...
$$;
revoke all on function tennis.validate_match_integrity() from public, anon, authenticated;
create trigger matches_integrity before insert or update on tennis.matches
for each row execute function tennis.validate_match_integrity();

create or replace function tennis.validate_match_set()
returns trigger language plpgsql set search_path = '' as $$
declare maximum_sets integer;
begin
  select best_of into maximum_sets from tennis.matches where id = new.match_id;
  if new.set_number > maximum_sets then raise exception 'set exceeds match format'; end if;
  return new;
end;
$$;
revoke all on function tennis.validate_match_set() from public, anon, authenticated;
create trigger match_sets_format before insert or update on tennis.match_sets
for each row execute function tennis.validate_match_set();

create or replace function tennis.generate_tournament_draw(target_edition_id uuid)
...
begin
  ...
  select * into edition from tennis.tournament_editions where id = target_edition_id for update;
  ...
  if exists (select 1 from tennis.matches where tournament_edition_id = target_edition_id) then raise exception 'draw already exists'; end if;
  select count(*) into entry_count from tennis.tournament_entries where tournament_edition_id = target_edition_id and entry_status in ('registered', 'active');
  if entry_count <> edition.draw_size then raise exception 'expected % entries, found %', edition.draw_size, entry_count; end if;
  if exists (select 1 from tennis.tournament_entries where tournament_edition_id = target_edition_id and seed is not null and seed > edition.draw_size) then raise exception 'seed outside draw'; end if;

  slots := array_fill(null::uuid, array[edition.draw_size]);
  for item in select player_id, seed from tennis.tournament_entries where tournament_edition_id = target_edition_id and entry_status in ('registered', 'active') and seed is not null order by seed loop
  ...
  select array_agg(player_id order by player_id) into remaining from tennis.tournament_entries where tournament_edition_id = target_edition_id and entry_status in ('registered', 'active') and seed is null;
  ...
  for level in 1..round_count loop
    ...
    for match_index in 1..match_count loop
      insert into tennis.matches(tournament_edition_id, round, round_order, match_number, player1_id, player2_id, best_of)
  ...
  update tennis.matches source set next_match_id = target.id, next_slot = case when source.match_number % 2 = 1 then 1 else 2 end
  from tennis.matches target where source.tournament_edition_id = target_edition_id and target.tournament_edition_id = target_edition_id
  ...
  update tennis.tournament_entries set entry_status = 'active' where tournament_edition_id = target_edition_id and entry_status = 'registered';
end;
$$;
revoke all on function tennis.generate_tournament_draw(uuid) from public, anon;
grant execute on function tennis.generate_tournament_draw(uuid) to authenticated;
...
$$;
revoke all on function tennis.confirm_match_result(uuid) from public, anon;
grant execute on function tennis.confirm_match_result(uuid) to authenticated;
