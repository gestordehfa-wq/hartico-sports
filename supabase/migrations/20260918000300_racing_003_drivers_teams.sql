...
alter table racing.drivers enable row level security;
create unique index drivers_active_number_idx on racing.drivers (racing_number) where status = 'active' and racing_number is not null;
create trigger drivers_updated_at before update on racing.drivers for each row execute function racing.set_updated_at();

create table racing.teams (
  id uuid primary key default extensions.gen_random_uuid(),
...
alter table racing.teams enable row level security;
create trigger teams_updated_at before update on racing.teams for each row execute function racing.set_updated_at();
